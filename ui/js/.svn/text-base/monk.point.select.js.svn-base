 ...
 */
			...
			.bind({
				click: function (event) {
					var x = Math.round(event.pageX - $(this).offset().left),
						label, intsec, i, len, angle, h, y, b;

					angle = self.controller.model.opts.line.angle;

					h = $(this).height() / 2;
					y = event.pageY - $(this).offset().top;


					b = (h - y) / Math.tan((90 - angle) / (180 / Math.PI));
					x -= b;

					x = Math.round(x);

                    x = x < 10 ? 10 : x;
                    x = x > $(this).width() - 10 ? $(this).width() - 10 : x;

					intsec = new Intersection(x, false, self.controller.model.opts.line.angle);
	...
	Monk.PointSelect.method('createLabel', function (rect, readonly) {
        var self = this;
		var intersections = {
			begin	: new Intersection(rect.x, readonly, self.controller.model.opts.line.angle),
			end		: new Intersection(rect.x + rect.w, readonly, self.controller.model.opts.line.angle)
		};
 ...
 */
	...
	Intersection = function (x, readonly, angle) {
		this.x		= x;
		this.angle	= angle || 45;
		this.color	= 'green';
		this.hcorr	= 0;
	...
	Intersection.method('render', function (index, tracker, key, parent) {
        ...
        angle = 45;
        if(this.angle != 45)
		    angle = Math.round(this.angle / 10) * 10;
        angle = angle < 30 ? 30 : angle > 130 ? 130 : 30;
        angle = 90 - angle;
		this.hcorr = w / 2;
 ...
 */
		...
		image = $('<img />')
			.addClass('image')
//			.attr('src', 'css/images/line.' + self.color + '.png')
            .attr('src', 'css/images/line.' + self.color + '.' + angle + '.png')
			.width(w)
			.height(h)
			.appendTo(container);
