...
define('triesearch_dir', '/var/www/s4aaas.target-imedia.nl/triesearch');
define('image_unauth', '/var/www/s4aaas.target-imedia.nl/rest_prototype/rest-data/image-unauth');
define('nextprev_unauth', '/var/www/s4aaas.target-imedia.nl/rest_prototype/rest-data/nextprev-unauth');
define('list_pages', '/var/www/s4aaas.target-imedia.nl/rest_prototype/rest-data/listpages');
...
switch (isset($url[1]) ? $url[1] : '') {
    case 'index':
        restrict_params(array('offset', 'rows', 'institutions', 'collections', 'books', 'match', 'annotations', 'wordzonetypes'), array());
        $results = search();
        xml_index($results);
        break;
    case 'books':
        restrict_params(array(), array());
        xml_books(books());
        break;
    case 'post':
        //TEST GUI for sending GET and POST parameters.
        require 'index.php';
        break;
    case 'login':
        restrict_params(array(), array('password'));
        login();
        break;
    case 'image_unauth':
        restrict_params(array(), array());
        image_unauth();
        break;
    case 'suggestion_unauth':
        restrict_params(array('rows'), array());
        $results = suggestions();
        xml_suggestions($results);
        break;
    case 'nextprev_unauth':
        restrict_params(array(), array());
        nextprev_unauth();
        break;
    case 'list_pages':
        restrict_params(array('role'), array());
        list_pages();
        break;
...
function list_pages() {
    ...
    if ($xml->authtoken != 'vTqpQj8WuzixYV1GFQUm')
        status_code(401, 'Unknown authentication_token.');
    $xml = new DOMDocument();
    if (isset($_GET['role']) && $_GET['role'] == 'trainee') {
        $xml->load(list_pages . '/trainee.xml');
    } else if (isset($_GET['role']) && $_GET['role'] == 'transcriber') {
        $xml->load(list_pages . '/transcriber.xml');
    }else
        status_code(400, 'Unknown or missing role.');
    header('X-Compression: gzip');
    header('Content-Encoding: gzip');
    header('Content-type: text/xml');
    exit(gzencode($xml->saveXML()));
}

function nextprev_unauth() {
    global $url;
    if (!isset($url[2]) || !strlen($url[2]))
        status_code(400, 'institution string empty.');
    if (!isset($url[3]) || !strlen($url[3]))
        status_code(400, 'collection string empty.');
    if (!isset($url[4]) || !strlen($url[4]))
        status_code(400, 'book string empty.');
    if (!isset($url[5]) || !strlen($url[5]))
        status_code(400, 'page string empty.');
    $fpath = nextprev_unauth . '/' . $url[2] . '/' . $url[3] . '/' . $url[4] . '/' . $url[5];
    $xml = new DOMDocument();
    if (is_file($fpath))
        $xml->load($fpath);
    else
        $xml->load(nextprev_unauth . '/error');
    header('X-Compression: gzip');
    header('Content-Encoding: gzip');
    header('Content-type: text/xml');
    exit(gzencode($xml->saveXML()));
...
function image_unauth() {
    global $url;
    if (!isset($url[2]) || !strlen($url[2]))
        status_code(400, 'image-id string empty.');
    header('Content-Type: image/jpeg');
    readfile(image_unauth . '/' . $url[2] . '.jpg');
    exit;
}
...
function login() {
    global $url;
    if (!isset($url[2]) || !strlen($url[2]))
        status_code(400, 'Username string empty.');
    if (!isset($_POST['password']))
        status_code(400, 'No password parameter.');
    if (!strlen($_POST['password']))
        status_code(400, 'Password string empty.');
    $db = new db();
    $column = $db->query_column('SELECT COUNT(*) COUNT FROM USERS WHERE MONK_ID = ' . escapeshellarg($url[2]) . ' AND PASSWORD = ' . escapeshellarg($_POST['password']));
    $xml = new xmlWriter();
    $xml->openMemory();
    $xml->setIndent(True);
    ...
    if ($column[0] == 1) {
        ...
        $xml->writeElement('authentication_token', 'vTqpQj8WuzixYV1GFQUm');
    } else {
        $xml->writeElement('status', 'ERROR');
        $xml->writeElement('authentication_token', '');
    }
    $xml->endElement();
    header('X-Compression: gzip');
    header('Content-Encoding: gzip');
    ...
    foreach ($annotations as $a)
        if ($a == 'W')
            foreach ($wordzone_types as $w)
                $shellcmd .= '--key=' . escapeshellarg('W\t' . $w . '\t' . $p . $url[2] . $s . '\t') . ' ';
        else
            $shellcmd .= '--key=' . escapeshellarg('' . $a . '\t\t' . $p . $url[2] . $s . '\t') . ' ';
...
function suggestions() {
    ...
    $shellcmd .= '--key=' . escapeshellarg(strtolower($url[2])) . ' ';
...
function restrict_params($params_get, $params_post) {
    foreach ($_GET as $key => $value)
        if (!in_array($key, $params_get))
            status_code(400, 'Parameter: "' . $key . '" unknown.');
    foreach ($_POST as $key => $value)
        if (!in_array($key, $params_post))
            status_code(400, 'Parameter: "' . $key . '" unknown.');
}
